"""Build the final schema-9 clean payload with side-by-side NF4 launcher.

The canonical schema-9 verifier remains ``kaggle_clean_codegen.py``.  The NF4
launcher is copied as ``kaggle_clean_codegen_nf4.py`` and imports that verifier.
Keeping both files avoids self-import when the launcher is executed from the
packaged payload.  Every byte remains covered by the schema-9 manifest.
"""
...
def main() -> None:
    ...
    def copy2_with_nf4(source, destination, *args, **kwargs):
        ...
        if source_path == canonical_runner:
            ...
            original_copy2(
                runner_source,
                destination_dir / "kaggle_clean_codegen_nf4.py",
            )
            original_copy2(
                validator_source,
