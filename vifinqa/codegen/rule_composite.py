...
def _resolve_with_own_metrics(facts, tables, variants, encoder, min_score, route=None):
    ...
    for f in facts:
        ...
        r = resolve_fact(f, tables, own, encoder, min_score, route=route)
        if r is None:
            return out, 0.0
        out.append(r)
    if not out:
