...
class ResolvedFact:
    ...
    def expr(self) -> str:
        ...
        frag = _fragment(self.label)
        return (f"float({self.var}.loc[{self.var}['label'].str.contains("
                f"{frag!r}, case=False, regex=False, na=False) "
...
def _fragment(label: str, max_len: int = 40) -> str:
    return re.sub(r"\s+", " ", str(label)).strip()[:max_len]
...
def distinct_cells(resolved: list[ResolvedFact]) -> bool:
    return len({(r.report_id, r.table_pos, norm(r.label), r.col) for r in resolved}) == len(resolved)
